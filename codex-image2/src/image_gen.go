...
func httpStatusError(status int) string {
	...
	case http.StatusBadRequest:
		return "API rejected the image request; check model access and request parameters"
	case http.StatusUnauthorized:
		return "API authentication failed; run setup again and check the API key"
	case http.StatusForbidden:
		return "API access was denied; the account or subscription may not include gpt-image-2"
	case http.StatusNotFound:
		return "API endpoint or gpt-image-2 model was not found; check the API URL and subscription group"
	case http.StatusTooManyRequests:
		return "API rate limit or quota was exceeded"
	default:
...
func doRequest(client *http.Client, makeRequest func() (*http.Request, error), maxAttempts int) ([]byte, error) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if err == nil {
			...
			last = httpStatusError(resp.StatusCode)
...
func generate(prompt, out string, args commonArgs) (map[string]any, error) {
	config, err := resolveAPIConfig(!args.dryRun)
	if err != nil {
		return nil, err
	}
	return generateConfigured(prompt, out, args, config)
}

func generateConfigured(prompt, out string, args commonArgs, config apiConfig) (map[string]any, error) {
	...
	ep, err := endpoint(config.BaseURL, "generations")
	if err != nil {
		return nil, err
	}
...
func edit(prompt string, imagePaths []string, mask, out string, args commonArgs) (map[string]any, error) {
	...
	return editConfigured(prompt, imagePaths, mask, out, args, config)
}

func editConfigured(prompt string, imagePaths []string, mask, out string, args commonArgs, config apiConfig) (map[string]any, error) {
	if err := validateCommon(args); err != nil {
		return nil, err
	}
	...
	ep, err := endpoint(config.BaseURL, "edits")
	if err != nil {
		return nil, err
	}
...
func runSetup(argv []string) error {
	...
	input, err := runSetupDialog(setupDefaults())
	if err != nil {
		return err
	}
...
func runStatus(argv []string) error {
	...
	printJSON(configStatus())
	return nil
}

func runReset(argv []string) error {
	fs := flag.NewFlagSet("reset", flag.ContinueOnError)
	var confirmed bool
	fs.BoolVar(&confirmed, "yes", false, "confirm removal of saved Codex Image2 configuration")
	if err := fs.Parse(argv); err != nil {
		return err
	}
	if !confirmed {
		return errors.New("reset removes the saved API URL and API key; pass --yes to confirm")
	}
